...
do
    ...
    if [ "$bAddDoc" = true ]; then
	ab -n $nRequests -c $nConcurrency -s 60 -p single_event.json -T 'application/json' -e ab_out.csv https://localhost/x/add 1> tmpout.ab 2>error.del
    else
	ab -n $nRequests -c $nConcurrency -s 60 -p query_singleton.json -T 'application/json' -e ab_out.csv https://localhost/x/query 1> tmpout.ab 2>error.del
    fi
